...
function MongoMessages() {
  ...
  const fetchMessages = async () => {
    try {
      ...
      console.log('Messages from backend:', response.data.messages); // Add this for debugging
        ...
        {loading ? (
          ...
          messages.map((data, index) => (
            <div key={index} className="message-box">
              ...
              <p className="message-type">Type: {data.type}</p> 
              <p className="message-topic">Topic: {data.topicName || data.topic}</p> 
              <button className="delete-btn" onClick={() => deleteMessage(data._id)}>
